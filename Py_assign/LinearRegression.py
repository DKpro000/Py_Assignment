...
class Model:
    ...
    def gradient_descent_linear_regression(self, x_list, y_list, lr=0.01, epochs=1000):
        m, c = 0.0, 0.0
        n = len(x_list)
        for _ in range(epochs):
            dm = sum(-2*x*y + 2*m*x**2 + 2*c*x for x, y in zip(x_list, y_list))
            dc = sum(-2*y + 2*m*x + 2*c for x, y in zip(x_list, y_list))
            m -= lr * dm
            c -= lr * dc
        return m, c
    ...
    def print_output(self,ManagerWindow):
        ...
        return TableCreator(ManagerWindow,"Prediction of total demand for each ingredient in the next week (7 days):",("name","quantity","unit"),OutputList)